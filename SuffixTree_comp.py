...
class SuffixTree:
        ...
        while pos < len(p):
            if p[pos] not in self.nodes[node][1].keys():
                if pos == len(p) - 1:
                    ...
                    self.add_node(node, p[pos])#nao passamos a posicao do sufixo que por defeito e -1
    ...
    def suffix_tree_from_seq(self, text):
        ...
        for i in range(len(t)):
            self.add_suffix(t[i:], i)#comeca na posicao i e vai ate ao fim
    ...
    def get_leafes_below(self, node):
        res = []
        #É folha
        if self.nodes[node][0] >=0: #se não for maior entao e uma folha
